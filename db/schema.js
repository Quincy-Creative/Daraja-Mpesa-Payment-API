...
// 	id: serial('id').primaryKey(),
// 	host_id: uuid('host_id').notNull().references(() => profiles.id),
// 	remaining_balance: numeric('remaining_balance', { precision: 14, scale: 2 }).default('0').notNull(),
// 	withdrawn: numeric('withdrawn', { precision: 14, scale: 2 }).default('0').notNull(),
// 	created_at: timestamp('created_at').defaultNow(),
// 	updated_at: timestamp('updated_at').defaultNow(),
// });
...
// helper: pending_stk (maps checkoutRequestID -> booking/guest/host)
...
// helper: payout_requests (tracks outgoing B2C requests)
const payout_requests = pgTable('payout_requests', {
  ...
  status: text('status').default('initiated'), // initiated, sent, completed, failed, timeout
  created_at: timestamp('created_at').defaultNow(),
  updated_at: timestamp('updated_at').defaultNow(),
});
